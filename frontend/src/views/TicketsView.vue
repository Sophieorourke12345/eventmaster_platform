...
onMounted(async () => {
  try {
    ...
    tickets.value = await Promise.all(data.tickets.map(async (ticket) => ({
      ...ticket,
      qrCode: await QRCode.toDataURL(`eventspace://ticket/${ticket.verificationCode}`, {
        width: 280,
        margin: 2,
    ...
    <div v-else-if="tickets.length" class="ticket-list">
      <article v-for="ticket in tickets" :key="ticket.id">
        ...
        <div class="ticket-main">
          ...
          <span class="ticket-status" :class="{ used: ticket.checkedInAt }">{{ ticket.checkedInAt ? '✓ Checked in' : 'Ready for entry' }}</span>
        </div>
        <div class="ticket-qr">
