...
const auth = useAuthStore(); const router = useRouter(); const error = ref(''); const form = reactive({ email: '', password: '', remember: true })
async function submit() { error.value = ''; try { await auth.login(form); router.push('/events') } catch (err) { error.value = err.message } }
</script>
